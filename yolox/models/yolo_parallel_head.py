...
# !! superclass class parameter reg_convs no longer used
...
class DecoupledYOLOHead(YOLOXHead):
    ...
    def __init__(
        ...
        for i in range(len(in_channels)):
            self.stems.append(
                BaseConv(in_channels=int(in_channels[i] * width), out_channels=int(256 * width), ksize=1, stride=1, act=activation)
            )
            self.cls_convs.append(
                Sequential(
                        Conv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act=activation),
                        Conv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act=activation),
                )
            )
            ...
            self.reg_preds.append( #modified
                Sequential(
                        Conv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act=activation),
                        Conv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act=activation),
            ...
            self.obj_preds.append( #modified
                Sequential(
                        Conv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act=activation),
                        Conv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act=activation),
